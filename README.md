...
3. Review the `.github/workflow/deploy-to-control-plane.yml` file:
    ...
    - The `sed` command is used to substitute the `ORG_NAME`, `GVC_NAME`, `WORKLOAD_NAME` and `IMAGE_NAME_TAG` tokens within the YAML files in the `/cpln` directory.

4. Review the two files in the `/cpln` directory:,
    - Update the `name` variable in `cpln-gvc.yaml` using the value of your desired GVC name.
...
- The GVC definition must exists in its own YAML file. The `cpln apply` command running the file that contains the GVC definition must be executed before any child definition YAML files (workloads, identities, etc.) are executed.
